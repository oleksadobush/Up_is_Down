...
const SPRING_FORCE: int = 6090
const ACCELER_SPEED: int = 1200
...
func _physics_process(delta):
	if is_instance_valid(Player):
		enemy_velocity.x = (Player.position - position).normalized().x * ENEMY_SPEED 
		...
		if not sliding and not jumped and UnderRay.is_colliding() and WallRay.is_colliding():
			enemy_velocity.y -= JUMP_FORCE
			jumped = true
			
		set_velocity(enemy_velocity + Vector2.DOWN * gravity * delta)
		move_and_slide()
...
func spring_jump():
	velocity.y -= lerp(enemy_velocity.y, enemy_velocity.y + SPRING_FORCE, LERP_WEIGHT)
